...
const DailyOrdersChart = ({ data }) => {
  if (!data || !Array.isArray(data) || data.length === 0) {
    return (
      <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 h-80 flex items-center justify-center">
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 h-96 ">
      ...
      <div className="h-72">
        ...
        <LineChart data={data}>
          ...
          />
          <Line type="monotone" dataKey="daily_orders" stroke="#3B82F6" strokeWidth={2} />
        </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
